...
void draw_svg_nodes(FILE* f, TreeNode* node) {
    ...
    if (node->type == NODE_DISTANCE) {
        ...
        for (int i = 0; i < node->kit_count; i++) {
            ...
            int valid_strs = 0;
            for(int m = 0; m < marker_count; m++) if(kits[kit_idx].str_values[m] > 0 && kits[kit_idx].str_values[m] < 256) valid_strs++;
        ...
        if (printed_lines == 0) {
             fprintf(f, "<text x=\"%.1f\" y=\"%.1f\" font-family=\"Arial\" font-size=\"12\" font-weight=\"bold\" fill=\"#000\" text-anchor=\"middle\">%s</text>\n", x + BOX_W/2.0, y + 20, node->name);
        }
